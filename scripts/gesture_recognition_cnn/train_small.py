...
class CNNGestureRecognizer(nn.Module):
    ...
    def __init__(self, num_classes: int = 10, dropout_rate: float = 0.5):
        ...
        # 方法1：直接分类（参数量：40,970）
        self.fc_output = nn.Linear(16 * 16 * 16, num_classes)  # 4096 -> 10
        
        # 方法2：全局平均池化（参数量：仅170！）- 取消注释以使用
        # self.global_avg_pool = nn.AdaptiveAvgPool2d(1)  # 16x16x16 -> 16x1x1  
        # self.fc_output = nn.Linear(16, num_classes)     # 16 -> 10
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        x = self.fc_output(x)         # (batch_size, num_classes)
        
        # 方法2：全局平均池化版本（取消注释以使用）
        # x = self.global_avg_pool(x)  # (batch_size, 16, 1, 1)
        # x = x.view(-1, 16)           # (batch_size, 16)  
        # x = self.fc_output(x)        # (batch_size, num_classes)
